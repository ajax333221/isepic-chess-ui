...
-------- | ---------- | ------ | ----------- | -----------
**setKeyNavMode**(<br>*allowNavigation*<br>) | <ul><li>allowNavigation (Boolean)</li></ul> | - | No | Turns on/off the **key navigation mode** to allow/deny the navigation with the keyboard arrows.<br><br>The **key navigation mode** is initially turned off.<br><br>:large_orange_diamond:**Important:** the scrolling via the keyboard arrows get disabled when the mode is on.<hr>Examples:<ul><li>`IcUi.setKeyNavMode(true)`</li><li>`IcUi.setKeyNavMode(false)`</li></ul>
